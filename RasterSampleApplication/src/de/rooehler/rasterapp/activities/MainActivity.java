...
import de.rooehler.rasterapp.rasterrenderer.gdal.GDALFileRenderer;
import de.rooehler.rasterapp.rasterrenderer.mbtiles.MBTilesRenderer;
import de.rooehler.rasterapp.rasterrenderer.mbtiles.MbTilesDatabase;
...
public class MainActivity extends Activity {
	...
	private void setMapStyle(final SupportedType type, final String filePath) {
		...
		try{
			...
			case MBTILES:
				...
				MBTilesRaster mbTilesRaster = new MBTilesRaster(filePath);
				MBTilesRenderer mbTilesRenderer = new MBTilesRenderer(getBaseContext(), AndroidGraphicFactory.INSTANCE, mbTilesRaster);
			...
			case RASTER:
				...
				GDALRaster gdalRaster = new GDALRaster(filePath);
				...
				GDALFileRenderer gdalFileRenderer = new GDALFileRenderer(AndroidGraphicFactory.INSTANCE, gdalRaster);
	...
	public MapPosition getCenterForGDALFile(GDALRaster raster){
...
//        boolean drawerOpen = mDrawerLayout.isDrawerOpen(mDrawerList);
        		...
        		if(isEditableMap()){
        			SelectProjectionDialog.showProjectionSelectionDialog(MainActivity.this, new IProjectionSelected() {
						...
						public void selected(String proj) {
							...
							GDALFileRenderer renderer = ((GDALFileRenderer) ((RasterLayer) mapView.getLayerManager().getLayers().get(0)).getRasterRenderer());
							Log.d(TAG, "Selected : "+proj);
							tileCache.destroy();
							renderer.applyProjection(proj);
    ...
    boolean isEditableMap(){
 	   ...
 	   ((RasterLayer) mapView.getLayerManager().getLayers().get(0)).getRasterRenderer() instanceof GDALFileRenderer;
    }
}
