...
public class MbTilesDatabase extends SQLiteOpenHelper {
	...
	public int[] getMinMaxZoom() {
		...
		try {
			Cursor c = this.mDataBase.rawQuery("select value from metadata where name=?", new String[] { "minzoom" });
			if (!c.moveToFirst()) {
				c.close();
				return null;
			}
			...
			c.close();
			c = this.mDataBase.rawQuery("select value from metadata where name=?", new String[] { "maxzoom" });
