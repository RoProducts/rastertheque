	...
	public static ColorMap parseColorMapFile(final File file){
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			if (nNode.hasAttributes()) {
				...
				colors.add(new ColorMapEntry(Color.parseColor(nodeMap.item(0).getNodeValue()), Float.parseFloat(nodeMap.item(1).getNodeValue()), Float.parseFloat(nodeMap.item(2).getNodeValue())));	
			}
		}	
