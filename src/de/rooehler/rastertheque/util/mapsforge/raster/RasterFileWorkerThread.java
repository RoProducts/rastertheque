...
public class RasterFileWorkerThread extends PausableThread {
	...
	private final RasterFileRenderer rasterFileRenderer;
	private final JobQueue<RasterFileJob> jobQueue;

	public RasterFileWorkerThread(TileCache tileCache, JobQueue<RasterFileJob> jobQueue,
			RasterFileRenderer pRasterFileRenderer, Layer layer) {
		...
		this.rasterFileRenderer = pRasterFileRenderer;
		this.jobQueue = jobQueue;
	}

	@Override
	protected void doWork() throws InterruptedException {
		RasterFileJob rendererJob = this.jobQueue.get();
		try {
	...
	private void renderTile(RasterFileJob rendererJob) {
		long start;
		if (DEBUG_TIMING) {
			start = System.currentTimeMillis();
		}

		TileBitmap bitmap = this.rasterFileRenderer.executeJob(rendererJob);
