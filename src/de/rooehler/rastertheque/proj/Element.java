...
public final class Element {
    ...
    final Symbols symbols = Symbols.SQUARE_BRACKETS;
     ...
     */
    Element(final String text, final ParsePosition position) throws ParseException
    {
        /*
        ...
        do {
            ...
            if (!Character.isUnicodeIdentifierStart(text.charAt(lower))) {
                final Number number = parseNumber(text, position, symbols.numberFormat);
                if (number == null) {
    ...
    final Number parseNumber(String text, final ParsePosition position, NumberFormat numberFormat) {
        if (true/*SCIENTIFIC_NOTATION*/) {
            /*
    ...
    private ParseException unparsableString(final String text, final ParsePosition position) {
        ...
        String message = unparsableStringMsg(text, position.getIndex(), errorIndex);
     ...
     */
    private String unparsableStringMsg(String text, final int index, int errorIndex) {
    ...
    public void print(final PrintWriter out, final int level) {
        ...
        out.print(spaces(tabWidth * level));
        out.println(keyword);
        if (list == null) {
            return;
        }
        ...
        for (int j=0; j<size; j++) {
            ...
            if (object instanceof Element) {
                ...
                out.print(spaces(tabWidth * (level+1)));
                out.println(object);
            }
        }
    }
