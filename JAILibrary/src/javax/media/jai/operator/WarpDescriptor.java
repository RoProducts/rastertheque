 ...
 * 
 * <p>
 * The destination bounds may be specified by an {@link ImageLayout} hint provided via a
 * {@link RenderingHints} supplied to the operation. If no bounds are so specified, then the
 * destination bounds will be set to the minimum bounding rectangle of the forward mapped source
 * bounds calculated using {@link Warp#mapSourceRect(Rectangle)} or, failing that,
 * {@link Warp#mapSourcePoint(Point2D)} applied to the vertices of the source bounds. If forward
 * mapping by both methods is not viable, then an approximate affine mapping will be created and
 * used to determine the destination bounds by forward mapping the source bounds. If this approach
 * also fails, then the destination bounds will be set to the source bounds.
 * 
 * <p>
 * "Warp" defines a PropertyGenerator that performs an identical transformation on the "ROI"
 * property of the source image, which can be retrieved by calling the <code>getProperty</code>
 * method with "ROI" as the property name.
 * 
 * <p>
 * The parameter, "backgroundValues", is defined to fill the background with the user-specified
 * background values. These background values will be translated into background colors by the
 * <code>ColorModel</code> when the image is displayed. With the default value, <code>{0.0}</code>,
 * of this parameter, the background pixels are filled with 0s. If the provided array length is
 * smaller than the number of bands, the first element of the provided array is used for all the
 * bands. If the provided values are out of the data range of the destination image, they will be
 * clamped into the proper range.
 * 
 * <p>
 * It should be noted that this operation automatically adds a value of <code>Boolean.TRUE</code>
 * for the <code>JAI.KEY_REPLACE_INDEX_COLOR_MODEL</code> to the given <code>configuration</code> so
 * that the operation is performed on the pixel values instead of being performed on the indices
 * into the color map if the source(s) have an <code>IndexColorModel</code>. This addition will take
 * place only if a value for the <code>JAI.KEY_REPLACE_INDEX_COLOR_MODEL</code> has not already been
 * provided by the user. Note that the <code>configuration</code> Map is cloned before the new hint
 * is added to it. The operation can be smart about the value of the
 * <code>JAI.KEY_REPLACE_INDEX_COLOR_MODEL</code> <code>RenderingHints</code>, i.e. while the
 * default value for the <code>JAI.KEY_REPLACE_INDEX_COLOR_MODEL</code> is <code>Boolean.TRUE</code>
 ...
 * <td>DocURL</td>
 * <td>
 * http://java.sun.com/products/java-media/jai/forDevelopers/jai-apidocs/javax/media/jai/operator
 * /WarpDescriptor.html</td>
 * </tr>
 * <tr>
 * <td>Version</td>
 * <td>1.0</td>
...
public class WarpDescriptor extends OperationDescriptorImpl {
    ...
    private static final Class[] paramClasses = {
        javax.media.jai.Warp.class, javax.media.jai.Interpolation.class,
	double[].class
    };
